...
'ListString',choice_str,'ListSize',[500 100]);
% parametrizaciq po koeficient na demferirane

if(select_param == 1)
    v_start = input('Задайте скорост на движение V,в m/s ='); 
    mm_start = input('Задайте начална стойност на демфера в mm: ');
    mm_step = input('Задайте стъпка за нарастване на параметъра на демфера в mm: ');
    iterations = input('Задайте брой итерации за параметризиране на коеф. на демфера: ');
    x_params = zeros(iterations);
    x_params(1)=mm_start;
end;
% parametrizaciq po skorost
if(select_param == 2)
    mm_start = input('Задайте стойност на демфера в mm:'); 
    v_start = input('Задайте начална стойност на скоростта V, в m/s: ');
    v_step = input('Задайте стъпка за нарастване на скоростта V, в m/s: ');
    iterations = input('Задайте брой итерации за параметризиране на скоростта V: ');    
    x_params = zeros(iterations);
    x_params(1)=v_start;
elseif(select_param > 2)
    disp('Грека при избора!');
end;
close all;
v = v_start;
mm = mm_start;
gen_output = zeros(12001,iterations);
...
for it = 1:len_t
    ...
    Qpp(it,6) = (-(beta1*(sign(-d21p)*(abs(-el*d21p)^mm)*(el^(1-mm))...
        ...
        %interpolate the acceleration curve
        InterpolQ1pp = interp1q( t,Q1pp,t_disc);
        gen_output(501:length(InterpolQ1pp),itt)= InterpolQ1pp(501:length(InterpolQ1pp));
        
        if(select_param == 1)
            x_label_str = sprintf('Параметър mm начална старт = %0.2f, край = %0.2f', x_params(1),x_params(iterations));
            FileString = sprintf('Accelerations_param_damp_coeff_%0.2f_to_%0.2f_iterations_%u_v=%u.txt',x_params(1),x_params(iterations),iterations,v);
        else
            x_label_str = sprintf('Параметър скорост начална старт = %f, край = %0.2f', x_params(1),x_params(iterations));
            FileString = sprintf('Accelerations_param_speed_%u_to_%u_iterations_%u_m=%0.2f.txt',x_params(1),x_params(iterations),iterations,mm);
        end
    
end
 %---------------Запазване на получените резултати във файлове--------------
 dlmwrite(FileString, gen_output, '\t');