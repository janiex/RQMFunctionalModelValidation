...
%----------------------DEFAULT CASE ---------------------------------------
    ...
    grid on;
    ylabel('Level of ride index comfort value (Sperling)');
    %damper coefficient parametrization file is read
    if(select_param == 1)
        xlabel('Damping coefficient in mm ');
        title('Ride index comfort by Sperling (speed V = 20m/s)');
    %speed parametrization file is read
    elseif(2 == select_param)
    ...
    if(select_param == 1)
        xlabel('Damping coefficient in mm ');
        title('Ride quality factor by Sperling (speed V = 20m/s)');
    %speed parametrization file is read
    elseif(2 == select_param)
        xlabel('Speed V in m/s');
